...
def handle_delete_single_event_ao_select(ack, body, client, logger, context):
    ...
    events = DbManager.find_records(
        ...
        [
            ...
            vwMasterEvents.event_date > datetime.now(tz=pytz.timezone("US/Central")) - timedelta(weeks=1),
...
def handle_edit_ao_select(ack, body, client, logger, context):
    ...
    selected_channel_name = body["view"]["state"]["values"]["edit_ao_select"]["edit_ao_select"]["selected_option"][
        "text"
    ]["text"]
    ...
    if selected_channel not in [ao.ao_channel_id for ao in aos]:
        ...
        for ao in aos:
            new_option = {
                "text": {"type": "plain_text", "text": ao.ao_display_name, "emoji": True},
...
def handle_date_select_button(ack, client, body, logger, context):
    ...
    if response.success:
        ...
        top_message = "Sorry, there was an error of some sort; please try again or contact your local administrator / Weasel Shaker."
